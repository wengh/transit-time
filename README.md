...
5. **Serialize** — all data is written to a custom binary format. Before writing, the events in each service pattern are sorted by `(stop_index, time_offset)`: all events at a given stop are stored contiguously, and within that group they are in chronological order. This layout serves double duty — the router can binary-search on time within each stop's bucket at query time, and the resulting sorted, monotone-within-group columns compress extremely well. The numeric event columns (time offsets, stop indices, travel times, event chain pointers, bucket offsets, and route labels) are compressed with PCO (a fast integer/float codec that exploits small deltas between adjacent values). Route shape coordinates are also PCO-compressed. The assembled binary is then gzip-compressed for transfer; the browser decompresses it on the fly during download.
