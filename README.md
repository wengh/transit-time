...
6. **Serialize** — all data is written to a custom binary format, with several layers of sorting applied to improve both compression ratios and runtime locality.
   ...
   *Event ordering:* the events in each service pattern are sorted by `(stop_index, time_offset)` — all events at a given stop contiguous, chronological within the group. This lets the router binary-search by time within each stop's bucket at query time, and the sorted columns compress efficiently with Pcodec. All numeric event columns (time offsets, stop indices, travel times, chain pointers, route labels), the frequency-trip columns and shape coordinates are Pcodec-compressed.

   *Stop index:* one global sparse index lists, per stop, the patterns that serve it together with the slice of that pattern's events and frequency rows at the stop. Its size is proportional to the number of (pattern, stop) pairs actually served — a few MB at most — rather than patterns × stops, which for cities whose feeds define one calendar per service day (Berlin, Amsterdam, Paris) used to cost 50–200 MB of browser memory in per-pattern offset arrays.
...
**Rendering:** After routing, each node's travel time is sent to a WebGL shader that maps it to a color. Points are drawn at a size proportional to the map zoom level, producing a continuous-looking coverage surface. The shader runs as a MapLibre GL custom layer inside the map's own render loop, inserted beneath the basemap's label layers so street and place names stay legible over the isochrone. Basemaps are CARTO's vector tiles (Dark Matter, Positron, Voyager), with raster OpenStreetMap styles as alternatives. Route polylines are drawn using the GTFS shape data where available, falling back to straight-line segments between stops.
